...
const TaxTermPurchaseOrderModal = ({ open, onClose, onSave, defaultRows }) => {
  ...
  const loadTaxData = async () => {
    try {
      ...
      // ✅ normalize dropdown only
      const normalized = list.map((x) => ({
        TAX_CODE: String(x.TAX_CODE || x.TaxCode || ""),
  ...
  const handleAdd = () => {
    ...
    const newRow = {
      ...
      type: taxType,
      amount: Number(selected.PERCENT).toFixed(2),
    };
...
export default TaxTermPurchaseOrderModal;