...
const EnquiryForm = () => {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      enquiry_hed_ex: {
        enq_no: formData.enquiryNo || `ENQ${Date.now()}`,
        enq_dt: new Date(formData.date).toISOString(),
        ...
        ref_dt: new Date(formData.refDocDate).toISOString(),
        ...
        std: "string",
        prod_spec: "string",
        profcen_cd: localStorage.getItem("PROFCEN_CD") || "01",
        user_name: "admin",
    ...
    try {
      ...
      setRows([]);
      setSelectedRows([]);
      navigate("/material/sales-enquiry-table"); 
    } catch (error) {
  ...
  return (
    <Container maxWidth="xl">
      <Box className="breadcrumb" mb={2}>
        <Breadcrumb
          routeSegments={[{ name: "Sales" }, { name: "Enquiry Detail" }]}
        />
      </Box>
      ...
      <Box sx={{ background: "#fff", p: 3, borderRadius: 2 }}>
        ...
        <Box display="grid" gridTemplateColumns="repeat(4, 1fr)" gap={2}>
          ...
          <FormControlLabel control={<Checkbox />} label="Missing" />
