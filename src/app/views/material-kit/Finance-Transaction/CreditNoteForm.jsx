...
export default function CreditNoteForm() {
  ...
  const predefinedOptions = [
    ...
    "Discount adjustment entry",
    "Correction of excess billing",
    "Credit note for customer account",
  ];
    ...
    <Container maxWidth="xl">
      ...
      <Box sx={{ p: 3, borderRadius: 2 }}>
        <Box display="flex" justifyContent="space-between" mb={2} alignItems="center">
          <Typography variant="h5" fontWeight="bold">Credit Note</Typography>
        ...
        <Box mt={3}>
          <Typography variant="subtitle1" fontWeight="bold">Added Credit Notes</Typography>
          {records.map((rec) => (
            <Box key={rec.id} sx={{ p: 1, border: "1px solid #ccc", borderRadius: 1, mb: 1 }}>
              <Typography>
                {`${rec.category} | Cust: ${rec.customerCode} | Voucher: ${rec.voucherNo} | Date: ${rec.voucherDate} | Narration: ${rec.narration}`}
              </Typography>
            </Box>
          ))}
        </Box>
